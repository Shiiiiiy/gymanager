	        ...
	        <div class="row">  
	             <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
	                   <div class="input-group" id="uploadImg">
	                       ...
	                       <a class="btn purple" href="${rc.contextPath}/uploadFile/companyUser.xls" ><i class="icon-cog"></i>&nbsp;下载模板</a>
	                   </div>
	              </div>
	         </div>
	          <#if !list?exists>
						...
						<tbody> 
							<#list list as company>
								<tr class="companys">
										...
										<td class="dbclick"><div class="zjh company" name="cp_name">${company.cp_name!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cpLink">${company.cpLink!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cpLinkEmail">${company.cpLinkEmail!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cpLinkTel">${company.cpLinkTel!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cpPhone">${company.cpPhone!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cpOrgCode">${company.cpOrgCode!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cpSocialCode">${company.cpSocialCode!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cpLocation">${company.cpLocation!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="comment">${company.comment!''}</div></td>
								 		<td class="dbclick"><a onclick="deleteCompany(this);"  class="btn mini black"><i class="icon-trash"></i>&nbsp;删除</a>&nbsp;&nbsp;</td>
								</tr>
			...
			$.each(divs,function(){
		 	...
		 	//数据校验
		  	...
		  	if(form_validate()){ //前台校验
		  		...
		  		$.ajax({
					  type: "POST",
					  url: "${rc.contextPath}/userManage/opt-save/saveCompanUser.do",
					  data: {jsonStr:str},
					  dataType:"text",
					  success:function(msg){
					  		if(msg){
					  			window.location.href="${rc.contextPath}/userManage/opt-query/companyList";
					  		}else{
			...
			//校验数据长度
		 	$.each(objs,function(index,obj){
					...
					if($(this).attr("name") == "username"){
						tt.checkLength(obj,"用户名称",true,2,20);
					}	
					if($(this).attr("name") == "password"){
						tt.checkLength(obj,"用户密码",true,6,16);
					}
					if($(this).attr("name") == "cp_name"){
						tt.checkLength(obj,"企业名称",false,1,20);
					}
					if($(this).attr("name") == "cpLink"){
						tt.checkLength(obj,"联系人",false,1,10);
					}
					if($(this).attr("name") == "cpLinkTel"){
						tt.checkLength(obj,"手机号码",false,1,11);
					}
					if($(this).attr("name") == "cpPhone"){
						tt.checkLength(obj,"座机号码",false,1,13);
					}
					if($(this).attr("name") == "cpOrgCode"){
						tt.checkLength(obj,"组织机构代码",false,1,32);
					}
					if($(this).attr("name") == "cpSocialCode"){
						tt.checkLength(obj,"社会信用代码",false,1,32);
					}
					if($(this).attr("name") == "cpLinkEmail"){
						tt.checkLength(obj,"邮箱",false,1,20);
					}
					if($(this).attr("name") == "cpLocation"){
						tt.checkLength(obj,"所在地",false,1,32);
					}
				});
