...
<html>
	        ...
	        <div class="row">  
	             <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
	                   <div class="input-group" id="uploadImg">
	                       ...
	                       <a class="btn purple" href="${rc.contextPath}/uploadFile/personUser.xls" ><i class="icon-cog"></i>&nbsp;下载模板</a>
	                   </div>
	              </div>
	         </div>
	          <#if !list?exists>
	         ...
	         <#if list??>
				<table class="table table-bordered table-striped table-hover mytable" id="improtCompanytable" style="table-layout:fixed">
							...
							<tr>
								...
								<th><div class="zjh">真实姓名</div></th>
								<th><div class="zjh">性别</div></th>
								<th><div class="zjh">邮箱</div></th>
								<th><div class="zjh">手机号码</div></th>
								<th><div class="zjh">座机号码</div></th>
								<th><div class="zjh">证件类型</div></th>
								<th><div class="zjh">证件号码</div></th>
								<th><div class="zjh">联系地址</div></th>
						...
						<tbody> 
							<#list list as company>
								<tr class="companys" >
										...
										<td class="dbclick"><div class="zjh company" name="name">${company.name!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="gender">${company.gender!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="email">${company.email!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="tel">${company.tel!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="phone">${company.phone!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cardtype">${company.cardtype!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="cardnum">${company.cardnum!''}</div></td>
										<td class="dbclick"><div class="zjh company" name="linkAddress">${company.linkAddress!''}</div></td>
	...
	//td转换input
			...
			$.each(divs,function(){
		 	...
		 	//数据校验
		  	...
		  	if(form_validate()){ //前台校验
		  		...
		  		$.ajax({
					  type: "POST",
					  url: "${rc.contextPath}/userManage/opt-save/savePersonUser.do",
					  data: {jsonStr:str},
					  dataType: "text",
					  success:function(msg){
					  		if(msg){
					  			window.location.href="${rc.contextPath}/userManage/opt-query/personList?infoType=import";
					  		}else{
			...
			//校验数据长度
		 	$.each(objs,function(index,obj){
					...
					if($(this).attr("name") == "name"){
						tt.checkLength(obj,"真实姓名",false,1,20);
					}
					if($(this).attr("name") == "gender"){
						tt.checkLength(obj,"性别",false,1,1);
					}
					...
					if($(this).attr("name") == "cardnum"){
						tt.checkLength(obj,"证件号码",false,1,40);
					}
					if($(this).attr("name") == "email"){
						tt.checkLength(obj,"邮箱",false,1,20);
					}
					if($(this).attr("name") == "linkAddress"){
		...
		function goBacks(){
			window.location.href="${rc.contextPath}/userManage/opt-query/personList";
		}
		function sureExcel(){
			if($("#excelUrl").val()){
