        ...
        <form class="form-inline search_list" role="form"  id="addOrEditModel" method="post">
            <div class="infor-con">
	            		...
	            		<p class="clearfix"><label class="fl">标题</label><span class="fl" style="width:90%;border-left:#ddd solid 1px; margin-left:-21px;padding:0 18px;">${(gardenNews.newsTitle)!''}</span></p>
		       			...
		       			<p><label>分类</label><span>${(tmCode)!''}</span></p>
	    
		       			<p><label>位置</label><span>${(tparentCode)!''}</span></p>
		            	...
		            	<p><label>内容</label><span><a target="_blank" href="${rc.contextPath}/newsall/opt-query/newsAllDetail.do?id=${(gardenNews.id)!''}" >预览</a></span></p>

		       			<p><label>状态</label><span>${(tDic)!''}</span></p>
