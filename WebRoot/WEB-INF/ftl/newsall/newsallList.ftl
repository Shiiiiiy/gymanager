...
<body>
    <div class="content-box">
        ...
        <div class="page-header"><h2>
        	<#if moduleCode?? && moduleCode="NEWS_A">
        		企业资讯
        	</#if>
        	<#if moduleCode?? && moduleCode="NEWS_C">
        		技术创新动态
        	</#if>
        	<#if moduleCode?? && moduleCode="NEWS_D">
        		创新创业动态
        	</#if>
        <small class="font-en caps"></small></h2></div>
        <form class="form-inline search_list" role="form"  id="searchForm" method="post" action="${rc.contextPath}/newsall/opt-query/newsAllList.do">
...
<script type="text/javascript">
	...
	function changeStatus(newsId,value){
		...
		window.location.href="${rc.contextPath}/newsall/opt-update/changeStatus.do?newsId="+newsId+"&pageNo="+pageNo+"&pageSize="+pageSize+"&value="+value+"&moduleCode=${(moduleCode)!''}";
	}
	...
	function editNews(newsId){
		var pageNo = $("#pageNo").val();
		var pageSize = $("*[name='pageSize']").val(); 
		window.location.href="${rc.contextPath}/newsall/opt-addOrEdit/addOrEditNewsAll.do?newsId="+newsId+"&pageNo="+pageNo+"&pageSize="+pageSize;
	}
	
	
	//新增新闻
	function addNews(){
		window.location.href="${rc.contextPath}/newsall/opt-addOrEdit/addOrEditNewsAll.do?moduleCode=${(moduleCode)!''}";
	}
	
	//查看新闻
	function newsInfo(newsId){
		window.location.href="${rc.contextPath}/newsall/opt-query/newsAllInfo.do?moduleCode=${(moduleCode)!''}&newsId="+newsId;		
	}
	...
	function sure(str){
		window.location.href="${rc.contextPath}/newsall/opt-del/deleteNewsAll.do?moduleCode=${(moduleCode)!''}&newsId="+str;
	}
