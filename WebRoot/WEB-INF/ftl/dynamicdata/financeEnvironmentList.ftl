            ...
            <table class="table table-bordered table-striped table-hover mytable">
                <thead>
                    <tr>
                        ...
                        <th width="6%">序号</th>
                        <th width="12%">机构名称</th>
                        <th width="12%">本地是否设立分支机构</th>
                        <th width="10%">本地累计投资规模</th>
                        <th width="9%">联系人</th>
                        <th width="9%">电话</th>
                        <th width="8%">状态</th>
                        <th width="29%">操作</th>
                    ...
                    <#if page??>
						<#list page.list as techi>
							<tr>
								...
								<td><div class="zjh"><a onclick="info('${(techi.ID)!''}')" href="javaScript:void(0)">${(techi.FINANCE_NAME)!"" }</a></div></td>
								<td><div class="zjh">${(techi.IS_BRANCH)!"" }</div></td>
								<td><div class="zjh">${(techi.SCALE)!"0" }<#if techi.SCALE?? && techi.SCALE!=''>元</#if> </div></td>
								<td><div class="zjh">${(techi.LINKMAN)!"" }</div></td>
								<td><div class="zjh">${(techi.TEL)!"" }</div></td>
								<td>
...
<script type="text/javascript">
	...
	function editTechInnovation(id){
		var pageNo = $("#pageNo").val();
		var pageSize = $("*[name='pageSize']").val();
		window.location.href="${rc.contextPath}/dynamicdata/opt-addOrEdit/addOrEditFinanceEnvironment.do?id="+id+"&modelType=${modelType}"+"&pageNo="+pageNo+"&pageSize="+pageSize;
	}
	
	
	//新增新闻
	function addTechInnovation(){
		window.location.href="${rc.contextPath}/dynamicdata/opt-addOrEdit/addOrEditFinanceEnvironment.do?modelType=${modelType}";
	}
	
	function info(id){
		window.location.href="${rc.contextPath}/dynamicdata/opt-query/financeEnvironmentInfo.do?id="+id;		
	}
