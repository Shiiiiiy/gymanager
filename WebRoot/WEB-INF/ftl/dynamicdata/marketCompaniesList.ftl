                    ...
                    <#if page??>
						<#list page.list as techi>
							<tr>
								...
								<td><div class="zjh"><a onclick="techInnovationInfo('${(techi.ID)!''}')" href="javaScript:void(0)">${(techi.COMP_NAME)!"" }</a></div></td>
								<td><div class="zjh">${(techi.OF_INDUSTRY)!"" }</div></td>
								<td><div class="zjh">${(techi.BOURSE)!"" }</div></td>
								<td><div class="zjh">${(techi.MARKET)!"" }</div></td>
								<td><div class="zjh">${(techi.MARKET_TIME)!"" }</div></td>
								<td>
...
<script type="text/javascript">
	...
	function editTechInnovation(id){
		var pageNo = $("#pageNo").val();
		var pageSize = $("*[name='pageSize']").val();
		window.location.href="${rc.contextPath}/dynamicdata/opt-addOrEdit/addOrEditMarketCompanies.do?id="+id+"&modelType=${modelType}"+"&pageNo="+pageNo+"&pageSize="+pageSize;
	}
	
	
	//新增新闻
	function addTechInnovation(){
		window.location.href="${rc.contextPath}/dynamicdata/opt-addOrEdit/addOrEditMarketCompanies.do?modelType=${modelType}";
	}
	
	//查看新闻
	function techInnovationInfo(id){
		window.location.href="${rc.contextPath}/dynamicdata/opt-query/marketCompaniesInfo.do?id="+id;		
	}
