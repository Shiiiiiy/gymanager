    ...
    <div class="content-box">
        ...
        <div class="tabfix">
            ...
            <ul class="nav nav-tabs" role="tablist" >
                ...
                <#if !(parentCode?? && parentCode =="MODULE_E_MAIN") >
...
var flag = "${flag}";
	...
	function callPerComplete(obj,data){
		...
		if(json.result){ //&& json.pictureFile)
			//将返回的地址信息存入数据库
			window.location.href="${rc.contextPath}/pillarindustry/opt-save/saveresource?fileName="+json.fileNameInServer+"&fileType="+type+"&flag="+flag+"&parentCode="+$("#parentCode").val();
		}
	}
	...
	function editFile(fileId,fileTitle,url,urlType,sort){
		...
		if(flag=="2" && zhi=="MODULE_E_MAIN"){//运行分析图 
	...
	function sure(fileId){
		...
		if(tt.validate()){
			...
			window.location.href="${rc.contextPath}/pillarindustry/opt-update/updateFileTitle.do?fileId="+fileId+"&fileTitle="+encodeURI(encodeURI($("#fileTitle").val()))+"&url="+encodeURI(encodeURI($("#url").val()))+"&urlType="+urlType+"&flag="+flag+"&parentCode="+$("#parentCode").val()+"&sort="+$("#sort").val();
		}else{
	...
	function sure1(fileId){
		window.location.href="${rc.contextPath}/pillarindustry/opt-delete/deleteFile.do?fileId="+fileId+"&flag="+flag+"&parentCode="+$("#parentCode").val();
	}
	...
	function changeCode(){
		//切换模块刷新页面
		window.location.href="${rc.contextPath}/pillarindustry/opt-query/resource.do?parentCode="+$("#parentCode").val()
	}
