...
public class ImageUploadUtil {
    ...
    public static void ckeditor(HttpServletRequest request, HttpServletResponse response, String DirectoryName)
       ...
       // String imageContextPath = request.getContextPath() + "/" + DirectoryName + "/" + fileName;
        ...
        if(fileName == null ){
        	 out.println("window.parent.CKEDITOR.tools.callFunction(" + callback + ",'','该图片无效')");	
        }else{
        	out.println("window.parent.CKEDITOR.tools.callFunction(" + callback + ",'" +request.getContextPath() +"/back/news/preview?fileName="+fileName+ "','')");
        }
        out.println("</script>");
        out.flush();
        out.close();
